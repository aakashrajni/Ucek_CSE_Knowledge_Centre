    ...
    <input type="text" class="hidden-sm hidden-xs" id="header-search-field" placeholder="Search for Something">
              ...
              <div class="col-md-7">
                ...
                <div class="panel-body">
                                            ...
                                            <tbody id="tbodyid">
                                                 <?php
                                                        $myPDO = new PDO('mysql:host=localhost;dbname=dihubin_dih', 'dihubin_root', 'Dihroot@1');
                                                         ...
                                                         $query = $myPDO->query("SELECT * FROM astud WHERE year='2' AND ver='1' ORDER BY regno ASC");
                                                             $query->execute();
                                                             while($row = $query->fetch(PDO::FETCH_ASSOC)){
                                                    ?>
                                                <tr class="odd gradeX">
                                                    <td class="left" ><input class="id" type="checkbox" name="c" onclick="nos(this)"></td>
                                                ...
                                                <?php
                                                        $myPDO = new PDO('mysql:host=localhost;dbname=dihubin_dih', 'dihubin_root', 'Dihroot@1');
                                                         ...
                                                         $query = $myPDO->query("SELECT * FROM astud WHERE year='2' AND ver='0' ORDER BY regno ASC");
                                                             $query->execute();
                                                             while($row = $query->fetch(PDO::FETCH_ASSOC)){
                                                    ?>
                                                <tr class="odd gradeX">
                      ...
                      <span>     Number of Students Selected:<span id="sp"></span></span>
                  <div class="modal-footer">
    ...
    $('#myModal').modal('hide');
     ...
     if (ele.checked) {
         ...
         for (var i = 0; i < checkboxes.length; i++) {
          ...
          function nos(clicked)
          {
            if (clicked.checked == true)
            {
              nosel = nosel + 1;
              document.getElementById("sp").innerHTML = nosel;
            }
